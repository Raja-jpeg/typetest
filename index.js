...
let hs_text = document.querySelector(".init_hs");
...
let hs_group = document.querySelector(".hs");
...
function finishGame() {
  ...
  let wpm_array = [1,2];
  let hs = Math.max(...wpm_array);
  
    if(wpm > hs){
      hs_text.textContent = wpm;
      hs = hs_text.textContent;
      wpm_array.push(hs)
      hs_group.style.display = "block";

    }
    else 
    {
      hs_text.textContent = hs;
    }
